(() => {
  ...
  var _candidateSiteId = null;
  var _debugSiteId = null;
  ...
  var debugSiteInfo = document.getElementById("debugSiteInfo");
  var detectedButtonsList = document.getElementById("detectedButtonsList");
  ...
  var scanCandidatesBtn = document.getElementById("scanCandidatesBtn");
  ...
  async function promoteCandidateToLocalButton(candidate) {
    ...
    if (!siteId) {
      setStatus("Could not determine current site.", "err");
      return;
    }
  ...
  function renderDebugInfo(resp) {
    ...
    deduped.forEach((btn) => {
      ...
      labelEl.textContent = btn.label + (!btn.enabled ? " (disabled)" : "") + (btn.enabled && !btn.found ? " \u2014 not visible" : "") + (btn.isIframe ? " [iframe]" : "") + (btn.custom ? " (local)" : "");
    ...
    reader.onload = async (e) => {
      try {
        ...
        for (const [siteId, btns] of Object.entries(incoming)) {
          ...
          btns.forEach((btn) => {
            if (btn.selector) {
              customButtons[siteId].push({ ...btn, custom: true });
              count++;
            }
          });
        }
  ...
  saveToLocalBtn.addEventListener("click", async () => {
    ...
    if (!label) {
      setStatus("Enter a label first.", "err");
      saveToLocalLabel.focus();
      return;
    }
    if (!sel) {
      setStatus("No selector to save.", "err");
      return;
    }
    ...
    if (customButtons[siteId].some((b) => b.selector === sel)) {
      setStatus("Selector already saved.", "err");
      return;
    }
